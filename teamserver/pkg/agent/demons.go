...
func (a *Agent) TaskDispatch(RequestID uint32, CommandID uint32, Parser *parser.Parser, teamserver TeamServer) {
	...
	/* if the RequestID was not generated by the TS, refect the request */
	if a.IsKnownRequestID(RequestID) == false {
		logger.Warn(fmt.Sprintf("Agent: %x, unknown RequestID: %x. This is either a bug or malicious activity", AgentID, RequestID))
		return
	}
